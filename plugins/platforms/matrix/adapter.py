...
class MatrixAdapter(BasePlatformAdapter):
    ...
    async def _handle_text_message(
        ...
        if self._sas_verification is not None and body.strip().lower() in (
            "!verify", "verify", "!verify device", "/verify",
        ):
            if not await self._is_dm_room(room_id):
                await self.send(
                    room_id,
                    "Device verification is only available in a direct message (DM) "
                    "with the bot — please send `!verify` there.",
                )
                return
            try:
                ...
                await self.send(room_id, f"Verification failed: {exc}")
            return
